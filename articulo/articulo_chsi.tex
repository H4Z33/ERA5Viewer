...
{\large Raúl Alejandro Morales Rivera}
...
Se recuperaron variables horarias de superficie terrestre del conjunto de datos ERA5-Land del Centro Europeo de Previsiones Meteorológicas a Plazo Medio (ECMWF) mediante la interfaz de programación del Copernicus Climate Data Store (CDS API) para el periodo de 28~años comprendido entre el 1 de enero de~1998 y el 31 de diciembre de~2025, lo que totaliza aproximadamente 245\,107 pasos temporales horarios. Las tres variables nucleares seleccionadas son:

\begin{enumerate}
    \item \textbf{Temperatura a 2~metros ($t2m$):} Representa el forzamiento térmico y la disponibilidad de energía en la interfaz suelo-atmósfera (unidades: Kelvin, convertidas a~$^\circ$C).
    \item \textbf{Contenido volumétrico de agua en la capa~1 del suelo ($swvl1$, 0--7~cm):} Representa la disponibilidad de agua en la capa superficial del suelo (unidades: m$^3$/m$^3$).
    \item \textbf{Evaporación potencial ($pev$):} Representa la demanda evaporativa potencial de la atmósfera (unidades: metros, acumulada; valores negativos indican flujo desde la superficie).
\end{enumerate}
...
Para representar el estado integrado de estrés hídrico a escala diaria, se formuló un índice basado en el principio de balance hídrico. El estrés hídrico alcanza su máximo cuando la temperatura ($t2m$) es elevada, la magnitud de la evaporación potencial ($pev$) es alta y el contenido de agua en el suelo ($swvl1$) se encuentra agotado. Los promedios diarios de las tres variables se normalizaron mediante un escalador MinMax sobre el periodo completo de 28~años:
%
\begin{equation}
...
\end{equation}
%
donde $T_{2m,\,\text{norm}}$, $\theta_{\text{swvl1},\,\text{norm}}$ y $|E_{\text{pev}}|_{\text{norm}}$ son las variables normalizadas al intervalo $[0,\,1]$. De esta forma, $\text{CHSI}_{\text{target}} = 0$ indica estrés hídrico mínimo (condiciones húmedas y frescas) y $\text{CHSI}_{\text{target}} = 1$ indica estrés hídrico máximo (condiciones secas y cálidas). La inversión del término de humedad edáfica ($1 - \theta_{\text{swvl1},\,\text{norm}}$) garantiza que valores bajos de contenido de agua contribuyan positivamente al índice de estrés.

\subsection{Derivación de variables predictoras}
\label{subsec:features}

A partir del conjunto de datos remuestreado a escala diaria se derivaron 23~variables predictoras diseñadas para capturar la dinámica temporal, las tasas de cambio, los cocientes físicos y las desviaciones climatológicas:

\begin{enumerate}
    \item \textbf{Cocientes físicos:} Índice térmico-hídrico $T_{2m} / (\theta_{\text{swvl1}} + \epsilon)$ y cociente de demanda evaporativa respecto a la humedad $|E_{\text{pev}}| / (\theta_{\text{swvl1}} + \epsilon)$, donde $\epsilon = 10^{-6}$ previene la división por cero.
    \item \textbf{Estadísticos móviles:} Media y desviación estándar en ventanas de 7~días para $t2m$, $swvl1$ y $pev$.
    \item \textbf{Derivadas temporales:} Diferencias a 1~día ($\Delta_{1d}$) y 7~días ($\Delta_{7d}$) para cada variable, representando las tasas de cambio de corto plazo.
    \item \textbf{Anomalías climatológicas mensuales:} Desviación de cada variable respecto a su media mensual de largo plazo (28~años).
    \item \textbf{Codificación cíclica del calendario:} Transformaciones seno y coseno del mes y del día del año (DOY) para capturar los ciclos estacionales de forzamiento solar.
\end{enumerate}

\subsection{Entrenamiento y comparación de modelos}
\label{subsec:modelos}

Para modelar las relaciones no lineales entre las variables predictoras y la variable de respuesta se evaluaron dos arquitecturas de ensamble: un \textbf{Regresor de Bosque Aleatorio} (RF, por \emph{Random Forest}; método de \emph{bagging}) y un \textbf{Regresor de Gradient Boosting} (GBR; método de \emph{boosting}). Ambos modelos se configuraron con 200~estimadores. Para el RF se estableció una profundidad máxima de~15 y un tamaño mínimo de hoja de~10 para prevenir el sobreajuste. Para el GBR se seleccionó una profundidad máxima de~8, una tasa de aprendizaje de~0,05 y una fracción de submuestreo de~0,8.

Para garantizar una validación robusta y prevenir la fuga temporal (\emph{temporal leakage}), se empleó una \textbf{validación cruzada de series temporales} (\texttt{TimeSeriesSplit} con $k = 5$ pliegues). El conjunto de datos se dividió de forma secuencial, asegurando que el modelo siempre se evaluara sobre datos futuros respecto a su conjunto de entrenamiento.
